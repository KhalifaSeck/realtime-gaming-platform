"""
Sink ADLS pour raw events (non aggreges).

Path : {container}/streaming_events/{topic}/date=YYYY-MM-DD/batch_*.parquet
Ces Parquet seront charges dans RAW.STREAM_* via COPY INTO.
"""
...
def make_writer(topic: str) -> Callable:
    def _write(batch_df: DataFrame, batch_id: int) -> None:
        if not ADLS_ACCOUNT or not ADLS_KEY:
            print(f"[adls_raw_events] skip batch {batch_id}: ADLS_ACCOUNT_NAME/KEY missing")
            return
        ...
        remote_path = f"streaming_events/{topic}/date={date_part}/batch_{batch_id}_{ts_ms}.parquet"

        fs = _adls_client().get_file_system_client(ADLS_CONTAINER)
        fs.get_file_client(remote_path).upload_data(buffer, overwrite=True)

        print(f"[adls_raw_events] batch {batch_id} -> {remote_path} ({len(pdf)} rows) topic={topic}")
